"""Run XInt-Bench against a pipeline and emit a vulnerability report (the product entry
...
def experiment(ctx) -> RunResult:
    ...
    return RunResult(summary=report.to_dict(), floats=[{
        ...
        "path": str(out), "claim": "XInt-Bench vulnerability report: attack efficacy + monitor detection per attack"}])
